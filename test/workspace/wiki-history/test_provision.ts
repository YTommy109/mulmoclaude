...
describe("provisionWikiHistoryHook — first install", () => {
  it("creates settings.json + hook script with our PostToolUse entry", async () => {
    ...
    // Hook is built by `yarn build:hooks` from
    // `server/workspace/wiki-history/hook/snapshot.ts` (esbuild
    // bundle). Anchor on the shebang and the snapshot endpoint
    // path — both survive any future bundle reformatting.
