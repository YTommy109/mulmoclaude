...
| **L-28** Docker sandbox 内で git/gh 認証通る (B-06) | ✅ 実装済 | docker.spec.ts、 sandbox enabled gate に加えて `status.sshAgent` または `status.mounts.includes("gh")` の credential-bridge gate → credential が 1 つも attach されてなければ「テストしたいシナリオではない」 として skip。 agent prompt は「Use the Bash tool to run `gh auth status` (no arguments). Then quote gh's stdout and stderr verbatim in your reply.」 で narration を最小化 → (a) `readSessionToolCalls(sessionId)` の `tool_call` jsonl を `bashCommandFromCall` でフィルタし、 `command` arg が `gh auth status` を含む `Bash` 呼出が 1 件以上あることを assert (LLM が prior knowledge から output 文字列を捏造する false-pass を塞ぐ、 Codex iter-1 / PR #1462)、 (b) 正: `[data-testid="text-response-assistant-body"].last()` に `/Logged in to github\.com/i` を `toContainText`、 (c) 負: 同 scope で `/not logged into any (?:GitHub )?hosts/i` を `not.toContainText` (新旧 gh の wording 差を吸収) の 3 軸 assert。 fake-echo は Bash dispatch を再現できないので per-test に `E2E_LIVE_NO_LLM=1` skip |
