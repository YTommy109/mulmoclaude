...
| **L-23** X MCP が Docker 内で .env から key を読める (B-01) | ✅ 実装済 | docker.spec.ts、 `getSandboxStatus(page)` で `/api/sandbox` を叩いて `null` (= sandbox disabled) なら test.skip → 続けて `getMcpToolsList(page)` で `/api/mcp-tools` catalog を取り `readXPost.enabled === true` + `searchX.enabled === true` を assert。 `requiredEnv: ["X_BEARER_TOKEN"]` の宣言保持も同 assert で carry-along チェック。 host env に `X_BEARER_TOKEN` 未設定だと L-23 が証明したい「.env propagation」 ではなく「未設定」 を観測することになるので precondition gate で skip (`enabled: false` 観測時)。 fake-echo backend では fake 不能 (catalog は host MCP registry の素 read) のため per-test に `E2E_LIVE_NO_LLM=1` skip。 spec ファイルは `.github/workflows/e2e_live_no_llm.yaml` matrix に **意図的に登録していない** (fake-friendly な test が 1 つもないため、 `docs/e2e-live-testing.md` の matrix 規約に従う) |
...
| **L-28** Docker sandbox 内で git/gh 認証通る (B-06) | ✅ 実装済 | docker.spec.ts、 sandbox enabled gate に加えて (`status.sshAgent || status.mounts.includes("gh")`) の credential-bridge gate → credential が 1 つも attach されてなければ「テストしたいシナリオではない」 として skip。 agent prompt は「Use the Bash tool to run `gh auth status` (no arguments). Then quote gh's stdout and stderr verbatim in your reply.」 で narration を最小化 → 応答 DOM に対して (a) 正: `/Logged in to github\.com/i` が visible (`.first()` で strict-mode 退避)、 (b) 負: `/not logged into any (?:GitHub )?hosts/i` が `toHaveCount(0)` (新旧 gh の wording 差を吸収)、 を 2 軸 assert。 fake-echo は Bash dispatch を再現できないので per-test に `E2E_LIVE_NO_LLM=1` skip |
| L-10, L-13, L-17, L-24, L-25, L-27, L-29, L-30 | 未実装 | 後続 PR で順次。 L-10 / L-13 はサーバ再起動 (env unset / 再接続) が必要なので別インフラ skill で扱う。 L-17 は `00f4a740 fix(notifier): drop HTTP publish` で外部から bridge message を注入するルートが廃止されており、 test 用 inject 経路 (engine.publish 直叩き or socket.io 直接 emit) の追加が前提。 L-24 / L-29 / L-30 は docker-only で別 PR、 L-25 / L-27 は manual-l4 (docs/manual-testing.md 待ち) |
