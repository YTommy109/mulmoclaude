...
- **Slug rule unified across journal / todos / wiki / files** (#732). Single canonical `slugify` (`server/utils/slug.ts`) — non-ASCII inputs deterministically hash (16-char base64url SHA-256 prefix), ASCII inputs lowercase + hyphenate. `isValidSlug` and the slug-output cap both moved from 64 → 120 chars to fit longer titles. **Breaking** for two surfaces: (a) journal topic filenames with non-ASCII names switch from `topic.md` (which previously silently collapsed distinct Japanese topic names onto each other) to `<hash>.md`; (b) the default Todo column id `in_progress` becomes `in-progress` and newly-created custom column ids use `-` instead of `_`. Existing `columns.json` ids on disk are read as-is, so existing workspaces keep working in a mixed-separator state. No migration script — orphaned journal summary files are regenerable from the conversation log.
