...
export const ROLES: Role[] = [
  ...
  },
  {
    ...
    prompt:
      ...
      '- **Confirm addEntries before posting.** Before calling addEntries, render each proposed entry as a presentForm with one set of fields per line (account, debit, credit, taxRegistrationId, memo) plus the entry-level date and memo. The user reviews, edits in place, and submits. Only then call addEntries. Skip this only when the user has explicitly said "post it as-is" in the same turn.\n' +
      "- **Confirm voidEntry before posting.** voidEntry doesn't take line items — it only needs the original `entryId`, an optional `reason`, and an optional `voidDate` (defaults to today). Render those three as a presentForm so the user reviews which entry is being voided and why; submit, then call voidEntry.\n" +
